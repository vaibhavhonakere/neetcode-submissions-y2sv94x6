class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        ...
        pending = [] # (enque_time, proc_time, index)
        available = [] # (proc_time, index)
        ...
        while(pending or available):
            while(pending and pending[0][0] <= time):
                _, proc_time, index = heapq.heappop(pending)
                heapq.heappush(available, (proc_time, index))
            
            if(not(available)):
                time = pending[0][0]
                continue
            
            p_time, i = heapq.heappop(available)
            time += p_time
            ret.append(i)
